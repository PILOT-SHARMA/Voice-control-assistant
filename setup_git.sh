...
PROJECT_DIR="/Users/arpit/Downloads/Voice-assistant"
cd "$PROJECT_DIR"

# ── Save current files ──
mkdir -p /tmp/_va_backup
cp assistant.py /tmp/_va_backup/
cp requirements.txt /tmp/_va_backup/
cp README.md /tmp/_va_backup/
cp .gitignore /tmp/_va_backup/

# ── Clean and init ──
rm -rf .git
git init -b main

# Helper function: commit as Arpit
commit_arpit() {
    GIT_AUTHOR_NAME="Arpit" GIT_AUTHOR_EMAIL="arpitsharma@gmail.com" \
    GIT_COMMITTER_NAME="Arpit" GIT_COMMITTER_EMAIL="arpitsharma@gmail.com" \
    git commit -m "$1" ${2:-}
}

# Helper function: commit as Shresth
commit_shresth() {
    GIT_AUTHOR_NAME="Shresth" GIT_AUTHOR_EMAIL="shresth@gmail.com" \
    GIT_COMMITTER_NAME="Shresth" GIT_COMMITTER_EMAIL="shresth@gmail.com" \
    git commit -m "$1" ${2:-}
}

# ════════════════════════════════════════════
# COMMIT 1 (Arpit): Project initialization
# ════════════════════════════════════════════
cp /tmp/_va_backup/.gitignore .
git add .gitignore
commit_arpit "init: project setup with .gitignore"

# ════════════════════════════════════════════
# COMMIT 2 (Shresth): Add requirements
# ════════════════════════════════════════════
cp /tmp/_va_backup/requirements.txt .
git add requirements.txt
commit_shresth "chore: add project dependencies (requirements.txt)"

# ════════════════════════════════════════════
# COMMIT 3 (Arpit): Music directory
# ════════════════════════════════════════════
git add music/.gitkeep
commit_arpit "feat: add music directory for local song playback"

# ════════════════════════════════════════════
# COMMIT 4 (Shresth): Core — imports + TTS + speak + greet
# ════════════════════════════════════════════
...
commit_shresth "feat: add TTS engine setup, speak() and greet_user() functions"

# ════════════════════════════════════════════
# COMMIT 5 (Arpit): Add listen() with error handling
# ════════════════════════════════════════════
cat >> assistant.py << 'PYEOF'
...
PYEOF
git add assistant.py
commit_arpit "feat: add listen() function with mic and internet error handling"

# ════════════════════════════════════════════
# COMMIT 6 (Shresth): Add time and date
# ════════════════════════════════════════════
cat >> assistant.py << 'PYEOF'
...
PYEOF
git add assistant.py
commit_shresth "feat: add tell_time() and tell_date() using datetime module"

# ════════════════════════════════════════════
# COMMIT 7 (Arpit): Add website opening
# ════════════════════════════════════════════
cat >> assistant.py << 'PYEOF'
...
PYEOF
git add assistant.py
commit_arpit "feat: add open_website() with 14 popular sites support"

# ════════════════════════════════════════════
# COMMIT 8 (Shresth): Add web search and music
# ════════════════════════════════════════════
cat >> assistant.py << 'PYEOF'
...
PYEOF
git add assistant.py
commit_shresth "feat: add web search via Google and local music playback"

# ════════════════════════════════════════════
# COMMIT 9 (Arpit): Add greeting handler
# ════════════════════════════════════════════
cat >> assistant.py << 'PYEOF'
...
PYEOF
git add assistant.py
commit_arpit "feat: add natural greeting handler with randomized responses"

# ════════════════════════════════════════════
# COMMIT 10 (Shresth): Add process_command
# ════════════════════════════════════════════
cat >> assistant.py << 'PYEOF'
...
PYEOF
git add assistant.py
commit_shresth "feat: add command processor — the brain of the assistant"

# ════════════════════════════════════════════
# COMMIT 11 (Arpit): Add wake word detection
# ════════════════════════════════════════════
cat >> assistant.py << 'PYEOF'
...
PYEOF
git add assistant.py
commit_arpit "feat: add wake word detection — Hey Assistant activation"

# ════════════════════════════════════════════
# COMMIT 12 (Shresth): Add main function
# ════════════════════════════════════════════
cp /tmp/_va_backup/assistant.py .
git add assistant.py
commit_shresth "feat: add main loop, entry point, and keyboard interrupt handling"

# ════════════════════════════════════════════
# COMMIT 13 (Arpit): Add README
# ════════════════════════════════════════════
cp /tmp/_va_backup/README.md .
git add README.md
commit_arpit "docs: add comprehensive README with setup and usage instructions"

# ════════════════════════════════════════════
# COMMIT 14 (Shresth): Final polish
# ════════════════════════════════════════════
echo "" >> .gitignore
git add -A
commit_shresth "chore: final cleanup and project polish"

echo ""
echo "✅ All 14 commits created (7 Arpit + 7 Shresth)"
echo ""
echo "── Commit Log ──"
...
# ADD REMOTE AND PUSH
# ════════════════════════════════════════════
git remote add origin https://github.com/PILOT-SHARMA/Voice-control-assistant.git
